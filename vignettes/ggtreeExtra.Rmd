...
p2 <- p1 + geom_star(mapping=aes(fill=Location, size=Length, starshape=Group),
                     starstroke=0.2) +
           ...
           scale_starshape_manual(values=c(1, 15),
                                  guide=guide_legend(keywidth=0.5, # adjust width of legend
...
p2 <- p + geom_fruit(data=dat1,
                     ...
                     position="identity",
                     starstroke=0.2) + 
          scale_size_continuous(range=c(1, 3), # the range of size.
                                guide=guide_legend(keywidth=0.5, keyheight=0.5,
                                                   override.aes=list(starshape=15),
                                                   order=2)) +
          scale_fill_manual(values=c("#F8766D", "#C49A00", "#53B400", "#00C094", "#00B6EB", "#A58AFF", "#FB61D7"),
...
p3 <- p2 + new_scale_fill() + 
          ...
          geom_axis_text(angle=-45, hjust=0) # add axis text of the layer.
p3
...
p4 <- p3 + new_scale_fill() +
           ...
           scale_alpha_continuous(range=c(0, 0.4), # the range of alpha
                                  guide=guide_legend(keywidth=0.5, keyheight=0.5, order=5)) +
           geom_axis_text(angle=-45, hjust=0) # add axis text of the layer.

# Then we add a bar layer outside of the tree.
p5 <- p4 + new_scale_fill() +
           geom_fruit(data=dat1, # The abundance of dat1 will be mapped to x, 
                      geom=geom_bar,
...
p2 <- p1 + geom_fruit_list(geom_fruit(data = df,
                                      geom = geom_bar,
